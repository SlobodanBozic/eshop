...
class ProductsController extends Controller
{
	public function addProduct(Request $request){
      ...
      if(!empty($data['product_owner'])){
        $product->product_owner = $data['product_owner'];
      }else{
      ...
      if(empty($data['feature_item'])){
        ...
        $conditionArray = array('New','Used');

        $product_ownerArray = array('Checked','Plain','Printed','Self','Solid');

		return view('admin.products.add_product')->with(compact('categories_drop_down','conditionArray','product_ownerArray'));
	}
	...
	public function editProduct(Request $request,$id=null){
		...
		if($request->isMethod('post')){
			...
			'product_code'=>$data['product_code'],'product_color'=>$data['product_color'],'description'=>$data['description'],'material'=>$data['material'],'price'=>$data['price'],'list_price'=>$list_price,'weight'=>$data['weight'],'image'=>$fileName,'video'=>$videoName,'product_condition'=>$condition,'product_owner'=>$product_owner]);
		...
		return view('admin.products.edit_product')->with(compact('productDetails','categories_drop_down','conditionArray','product_ownerArray'));
	}
    ...
    public function products($url=null){
        ...
        if(!empty($_GET['condition'])){
            $conditionArray = explode('-',$_GET['condition']);
            $productsAll = $productsAll->whereIn('products.product_condition',$conditionArray);
        }

        if(!empty($_GET['product_owner'])){
            $product_ownerArray = explode('-',$_GET['product_owner']);
            $productsAll = $productsAll->whereIn('products.product_owner',$product_ownerArray);
        }
        ...
        $conditionArray = Product::select('product_condition')->where('product_condition','!=','')->groupBy('product_condition')->get();
        $conditionArray = array_flatten(json_decode(json_encode($conditionArray),true));

        $product_ownerArray = Product::select('product_owner')->where('product_owner','!=','')->groupBy('product_owner')->get();
        $product_ownerArray = array_flatten(json_decode(json_encode($product_ownerArray),true));
        ...
        $meta_description = $categoryDetails->meta_description;
    		...
    		return view('products.listing')->with(compact('categories','productsAll','categoryDetails','meta_title','meta_description','meta_keywords','url','colorArray','conditionArray','product_ownerArray','sizesArray','breadcrumb'));
    }
    ...
    public function filter(Request $request){
        ...
        $conditionUrl="";
        if(!empty($data['conditionFilter'])){
            foreach($data['conditionFilter'] as $condition){
                if(empty($conditionUrl)){
                    $sconditionUrl = "&condition=".$condition;
                }else{
        ...
        $product_ownerUrl="";
        if(!empty($data['product_ownerFilter'])){
            foreach($data['product_ownerFilter'] as $product_owner){
                if(empty($product_ownerUrl)){
                    $product_ownerUrl = "&product_owner=".$product_owner;
                }else{
                    $product_ownerUrl .= "-".$product_owner;
                }
            }
        }
        ...
        $finalUrl = "products/".$data['url']."?".$colorUrl.$conditionUrl.$product_ownerUrl.$sizeUrl;
    ...
    public function getProductPrice(Request $request){
        ...
        echo $proAttr->price."-".$getCurrencyRates['USD_Rate']."-".$getCurrencyRates['GBP_Rate']."-".$getCurrencyRates['BAM_Rate'];
        echo "#";
        echo $proAttr->stock;
    }
    ...
    public function addtocart(Request $request){
        ...
        $data = $request->all();
	       	...
	       	$product_size = $sizeIDArr[1];
					...
					if($wishListCount > 0){
						return redirect()->back()->with('flash_message_error','Product aleady exists in Wish List');
					}else{
					// Insert Product in Wish List
					DB::table('wish_list')->insert(['product_id' => $data['product_id'],'product_name' => $data['product_name'],
					 'product_code' => $data['product_code'], 'product_color' => $data['product_color'], 'price' => $product_price, 'size' => $product_size,
    ...
    public function placeOrder(Request $request){
        if($request->isMethod('post')){
            ...
            $shippingCharges = Product::getShippingCharges($shippingDetails->country);*/
						...
						$grand_total = Product::getGrandTotal(); // Secure Grand Total at Order placement

						Session::put('order_id', $grand_total);
				  ...
				  <body>
				    ...
				    <header class='clearfix'>
				      ...
				      <div id='company' class='clearfix'>
								<div><strong>Billing To</strong></div>
				      ...
				      <div id='project'>
								<div><strong>Shipping Adress</strong></div>
				        <div><span>Name: </span> $orderDetails->name</div>
								...
								<div><span>Mbbile: </span>$orderDetails->mobile</div>
				      </div>
				    </header>
				    ...
				    <main>
				      <table>
				        <thead>
				          <tr>
									<td class='text-center' style='width:18%'><strong>Product Code</strong></td>
									<td class='text-center' style='width:18%'><strong>Product Size</strong></td>
									<td class='text-center' style='width:18%'><strong>Color</strong></td>
									<td class='text-center' style='width:18%'><strong>Price</strong></td>
									<td class='text-center' style='width:18%'><strong>Qty</strong></td>
									<td class='text-center' style='width:18%'><strong>Totals</strong></td>
								</tr>
				          </tr>
				        </thead>
				        <tbody> ";
								...
								foreach($orderDetails->orders as $pro){
									$totals = $pro->product_price * $pro->product_qty;
						$output .="<tr>
						...
						$output .="
				          ...
				          <tr>
				            ...
				            <td class='total'>€ $orderDetails->shipping_charges</td>
									...
									<tr>
									 <td colspan='5'>COUPON DISCOUT (-)</td>
