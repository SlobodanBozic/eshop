...
class AdminController extends Controller
{
    ...
    public function addAdmin(Request $request){
      ...
      if($request->isMethod('post')){
        ...
        if($adminCount > 0 ){
          return redirect()->back()->with('flash_message_error','Admin already exists! Please choose anather.');
        }
        else{
        ...
        $data['status'] = 0;
          ...
          // If we are creating super Admin with full control
          if($data['type'] =="Admin"){
            $admin = new Admin;
          ...
          // We are creating sub admin with some  restricted access
          ...
          if(empty($data['users_access'])){
            $data['users_access'] = 0;
              ...
              $admin->password = md5($data['password']);
              $admin->status = $data['status'];
              $admin->categories_access = $data['categories_view_access'];
              $admin->categories_access = $data['categories_edit_access'];
              $admin->categories_access = $data['categories_full_access'];
    ...
    public function editAdmin(Request $request, $id){
      ...
      if($request->isMethod('post')){
        ...
        // Update accesses for Super Admin
          if($data['type'] =="Admin"){
          Admin::where('username', $data['username'])->update(['password' => md5($data['password']), 'status' => $data['status']]);
          return redirect()->back()->with('flash_message_success', 'Admin update successfully');
          }
